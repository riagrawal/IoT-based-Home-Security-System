...
sensor.controller('mainController', function ($scope, $http, Notification, $interval) {
        ...
        $scope.init = function () {
          ...
          $scope.switchStatus = $scope.currentSt.PiState;
          $scope.thrs = new Threshold($scope.currentSt.ThresVal);
      ...
      $scope.sensorCall = function(){
        ...
        .success(function(response) {
          ...
          $scope.sensorData = response;
           $scope.distData = ($scope.sensorData.Dist_Value / 100).toFixed(2);
           //console.log("Light data " + $scope.sensorData.Light)
          if($scope.sensorData.Light < 255 && $scope.sensorData.Light >=0){
            $scope.lightData = ((255 - $scope.sensorData.Light_Value) * 100 / 255).toFixed(2);
           }
